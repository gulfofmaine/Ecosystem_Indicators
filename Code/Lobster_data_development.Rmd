...
ALSI_bp <- ALSI %>% 
  nest(data = -c(stat_area, name, Season)) %>% 
  mutate(mod = map(data, ~bp_analysis(.x, npsi = 1)),
         breakpoint1 = purrr::map(mod, ~.x$psi[[2]])) %>% 
  select(stat_area, name, Season, breakpoint1) %>% 
  unnest(breakpoint1)
...
sublegal_cpue_bp <- sublegal_cpue %>% 
  nest(data = -c(stat_area, name, Season)) %>% 
  mutate(mod = map(data, ~bp_analysis(.x, 1)),
         breakpoint1 = purrr::map(mod, ~.x$psi[[2]])) %>% 
  select(stat_area, name, breakpoint1) %>% 
  unnest(breakpoint1)

sublegal_plot

knitr::kable(sublegal_cpue_bp)
```
...
MENH_lob_index_bp <- MENH_lob_index %>% 
...
ME_pounds <- ME_landings %>% 
  mutate(name = "ME_landings",
         Season = "all") %>% 
  select(Year, "lob_index" = Pounds, name, stat_area, Season) 
...
ME_pounds_bp <- ME_pounds %>% 
  ...
  select(stat_area, name, breakpoint1, breakpoint2) %>% 
...
NEFSCindex_bp <- NEFSCindex %>% 
  ...
  mutate(mod = map(data, ~bp_analysis(.x, 2)),
         breakpoint1 = purrr::map(mod, ~.x$psi[[1,2]]),
         breakpoint2 = purrr::map(mod, ~.x$psi[[2,2]])) %>% 
  select(stat_area, Sex, name, breakpoint1, breakpoint2) %>% 
  unnest(c(breakpoint1, breakpoint2))
...
NEFSC_lobIndex_bp <- NEFSC_lobIndex %>% 
  nest(data = -c(stat_area, Season, name)) %>% 
  mutate(mod = map(data, ~bp_analysis(.x, 2)),
         breakpoint1 = purrr::map(mod, ~.x$psi[[1,2]]),
         breakpoint2 = purrr::map(mod, ~.x$psi[[2,2]])) %>% 
  select(stat_area, Season, name, breakpoint1, breakpoint2) %>% 
  unnest(c(breakpoint1, breakpoint2))
...
```{r all_lob_data}
all_lob_data <- bind_rows(NEFSC_lobIndex, NEFSCindex, ME_pounds, MENH_lob_index, sublegal_cpue, ALSI)

write_csv(all_lob_data, here::here("Processed_Indicators/all_lob_data.csv"))

lob_breakpoints <- bind_rows(NEFSC_lobIndex_bp, NEFSCindex_bp, ME_pounds_bp, MENH_lob_index_bp, sublegal_cpue_bp, ALSI_bp)
