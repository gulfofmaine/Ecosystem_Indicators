...
```{r slope_segments_2}
bp <- slope_bp_fun(indicator_sel, season_sel, stat_areas_sel, 2)
summary(bp)
plot(bp)
df <- allIndicators %>% 
  filter(Indicator == indicator_sel,
         Season == season_sel,
         stat_area == stat_areas_sel)
points(x = df$Year, y = scale(df$Value))
```

#### Slope breakpoint (3)

```{r slope_segments_3}
bp <- slope_bp_fun(indicator_sel, season_sel, stat_areas_sel, 3)
summary(bp)
plot(bp)
df <- allIndicators %>% 
  filter(Indicator == indicator_sel,
