...
# Lobster Predators
lob_predators <- read_csv("/Users/mdzaugis/Box/Mills Lab/Projects/Lobster/indicators/Indicators/lobster_pred_list.csv")

### for size spec

MENH_catch <- read_csv("/Users/mdzaugis/Box/Mills Lab/Projects/Lobster/indicators/Indicators/MaineDMR_Catch_at_Length_Data_2020-07-13.csv", guess_max = 1e6)
...
```{r plots, echo = FALSE}
lob_size_spec %>% group_by(SpecCode, Year, LngtClass) %>% tally() %>% 
  ggplot() + geom_line(aes(Year, n, col = as.factor(LngtClass))) + 
  facet_wrap(~SpecCode) + theme_bw() + scale_color_discrete(name = "Length Class")

lob_size_spec %>% filter(Year == 2012) %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~SpecCode) + 
  labs(title = "2012", x = "Length Class") + theme_bw() + scale_y_continuous(limits = c(0, 26))

lob_size_spec %>% filter(Year == 2013) %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~SpecCode) + 
  labs(title = "2013", x = "Length Class") + theme_bw()+ scale_y_continuous(limits = c(0, 26))

lob_size_spec %>% filter(Year == 2014) %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~SpecCode) + 
  labs(title = "2014", x = "Length Class") + theme_bw()+ scale_y_continuous(limits = c(0, 26))

lob_size_spec %>% filter(Year == 2015) %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~SpecCode) + 
  labs(title = "2015", x = "Length Class") + theme_bw()+ scale_y_continuous(limits = c(0, 26))

lob_size_spec %>% filter(Year == 2016) %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~SpecCode) + 
  labs(title = "2016", x = "Length Class") + theme_bw()+ scale_y_continuous(limits = c(0, 26))


lob_size_spec %>% filter(Year == 2017) %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~SpecCode) + 
  labs(title = "2017", x = "Length Class") + theme_bw()+ scale_y_continuous(limits = c(0, 26))


lob_size_spec %>% filter(SpecCode == "Dogfish Spiny") %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~Year) + 
  labs(title = "Spiny Dogfish", x = "Length Class") + theme_bw()

lob_size_spec %>% filter(SpecCode == "Haddock") %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~Year) + 
  labs(title = "Haddock", x = "Length Class") + theme_bw()

lob_size_spec %>% filter(SpecCode == "Flounder Winter") %>% 
  group_by(SpecCode, LngtClass, Year) %>% 
  ggplot() + geom_histogram(aes(LngtClass)) + 
  facet_wrap(~Year) + 
  labs(title = "Winter Flounder", x = "Length Class") + theme_bw()
```
...
# to save results for each year
data.year.list = vector(mode = "list", length = length(fullYears)) %>% 
  setNames(fullYears)


####  same but in an apply function:
data.year.list  <- dataRecommend.isd %>% 
  ...
  map(function(x){
    ...
    wmin.vec = data.year$wmin    # Vector of lower bin limits
    wmax.vec = data.year$wmax    # Vector of upper bin limits
    num.vec  = data.year$Number  # Vector of corresponding counts

    # to do a manual count
...
fullYears <- names(data.year.list)
#
...
```{r plots}
data.year.list$`2015` %>% ggplot() + 
  geom_linerange(aes(xmin = wmin, xmax = wmax, y = countGTEwmin)) + 
  geom_linerange(aes(wmin, ymin = lowCount, ymax = highCount)) +
  geom_linerange(aes(wmax, ymin = lowCount, ymax = highCount)) +
  scale_x_log10()
```
