...
# UpdateBuoy is a function that downloads temperature, salinity, 
  # and density data from six NERACOOS buoys (B, E, F, I, M, N) 
  # and produces a vector of daily means and the date for each buoy, depth, and variable. 
  # This document contains in line descrpitions of the three underlying functions needed 
  # to run the `UpdateBuoy` function: `UpdateBuoy`, `loadNERACOOSnetcdf`, and `surfbotcorr`.

# Below is the code for the `UpdateBuoy` function with descriptions for each of the code chuncks. 
  # This function calls on two other functions, `loadNERACOOSnetcdf` and `surfbotcorr`, and 
  # descriptions are under the respective headings.

#This function opens a downloaded netcdf file of a standard format, 
#`filename` and extracts the variable called in `varname`. 
#This function is called within `UpdateBuoys` and does not need to be modified by the user.
...
UpdateBuoy<-function(buoy, smoothing, surfbot){
  ...
  destfile <- paste(here::here("Data/Buoy_files/"), buoy,"_",Sys.Date(), ".nc", sep="")
...
#This function calculates the surface to bottom linear regression to fill in gaps in the 50m temperature data, 
#if the surface temperature is available. 
#`surfbotcorr` is called with a logical `TRUE` or `FALSE` by the user as an argument in the `UpdateBuoy` function.
...
#This function takes in the buoy of interest and asks if the data should be smoother or NA filled
